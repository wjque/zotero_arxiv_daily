...
async function main(){try{state.config=await(await fetch("data/site-config.json",{cache:"no-store"})).json();const raw=await(await fetch(`data/${state.config.data_file}`,{cache:"no-store"})).text();if(state.config.encrypted){const passphrase=window.prompt(state.t.passphrase);if(!passphrase)throw new Error(state.t.required);state.data=await decrypt(JSON.parse(raw),passphrase)}else state.data=JSON.parse(raw);if(![1,2,3].includes(state.data.schema_version)||!Array.isArray(state.data.recommendations))throw new Error("Unsupported recommendation schema.");localize();controls.hidden=false;selectOptions();batchStatus();for(const input of controls.querySelectorAll("input,select"))input.addEventListener("input",render);document.querySelector("#feedback-confirm").addEventListener("click",()=>{state.feedback={};saveFeedback()});updateFeedbackExport();render()}catch(error){status.textContent=state.t.unsafe;const message=document.createElement("p"),link=document.createElement("a");message.className="error";message.textContent=String(error.message);link.href="https://arxiv.org/";link.textContent=state.t.open;list.replaceChildren(message,link)}}main();
"""
