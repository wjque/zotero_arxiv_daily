...
function selectOptions(){const categories=[...new Set(state.data.recommendations.flatMap(item=>item.categories))].sort();document.querySelector("#category-filter").replaceChildren(new Option(state.t.allCategories,""),...categories.map(value=>new Option(value,value)));document.querySelector("#source-filter").replaceChildren(new Option(state.t.allSources,""),...['core','adjacent','exploration'].map(value=>new Option(value,value)));document.querySelector("#feedback-filter").replaceChildren(new Option(state.t.allFeedback,""),...actions.map(value=>new Option(state.t.actions[value],value)))}
