...
_ENVIRONMENT_KEYS = {
    ...
    "ZAD_PROFILE_FEATURE_KEY": "profile_feature_key",
...
class AppConfig:
    ...
    def validate(self) -> None:
        ...
        if (
            self.profile_feature_key is not None
            and len(self.profile_feature_key.encode("utf-8")) < 32
        ):
            raise ConfigurationError("profile_feature_key must contain at least 32 UTF-8 bytes")
...
def load_config(
    *,
    ...
    config = AppConfig(
        ...
        profile_feature_key=_optional_string_value(normalized_values, "profile_feature_key"),
