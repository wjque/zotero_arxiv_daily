...
def test_default_policy_covers_fields_without_trait_specific_rules() -> None:
    ...
    assert "absence must never lower a score" in instruction
...
def test_policy_registry_is_explicit_and_rejects_incomplete_versions() -> None:
    assert quality_reference_policy_versions() == (DEFAULT_QUALITY_REFERENCE_POLICY.version,)
    ...
    with pytest.raises(ValueError, match="every field role"):
        QualityReferencePolicy(
            "incomplete",
            "judge-v9",
            tuple(
